...
class OPT_attack_sign_SGD(object):
    ...
    def attack_untargeted(self, x0, y0, alpha = 0.2, beta = 0.005, iterations = 1000, query_limit=20000,
        ...
        G0 = to_networkx(x0, to_undirected=True)
        time_start = time()
        initial_theta, inital_F, initial_g, num_query, search_type = self.initial_search(x0, y0, node_num_origin)
        ...
        for i in range(iterations):
            ...
            sign_gradient, grad_queries = self.sign_grad_v1(x0, y0, theta, initial_F=F_theta, initial_g=g_theta, node_num_origin=node_num_origin)
            ...
            for _ in range(15):
                ...
                new_g, count = self.fine_grained_binary_search_local(model, x0, y0, new_theta, initial_lbd = min_g, node_num_origin=node_num_origin)
        ...
        target = model.predict(x_final, self.device)
        time_end = time()
        print(f"original label: {y0}, perturbed label: {target}")
